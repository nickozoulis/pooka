package speed.storm.bolt;

/**
 * Created by nickozoulis on 12/06/2016.
 */
public class Cons {
    public static final String TABLE_SPEED = "speed_views_exp";
    public static final String MASTER_DATASET = "master_dataset_exp";
    public static final String TABLE_BATCH = "batch_views_exp";
    public static final String CF_VIEWS = "v";
    public static final String CF_MASTER_DATASET_INFO = "i";
    public static final String CF_MASTER_DATASET_OTHER = "o";
    public static final String localhost = "127.0.0.1";
    public static final String zk_port = "2181";
    public static final String countPrefix = "count_";
    public static final String avgPrefix = "avg_";
    public static final String stdevPrefix = "stdev_";
}
